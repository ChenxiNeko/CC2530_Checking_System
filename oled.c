...
{
  oled_iic_start();						        //启动总线
  if(oled_iic_write_byte(ADDR_W) == 0){		        //地址设置
    if(oled_iic_write_byte(0x00) == 0){			        //命令输入
      oled_iic_write_byte(IIC_Command);	                                //等待数据传输完成
    }
  }
  oled_iic_stop();
}
...
void OLED_IIC_write(unsigned char IIC_Data)
{
  oled_iic_start();						        //启动总线
  if(oled_iic_write_byte(ADDR_W) == 0){		        //地址设置
    if(oled_iic_write_byte(0x40) == 0){			        //命令输入
      oled_iic_write_byte(IIC_Data);	                                //等待数据传输完成
    }
  }
  oled_iic_stop();
}
...
void OLED_ShowCHineseString(unsigned char x,unsigned char y,unsigned char num,unsigned char n)
{
  for(unsigned char i=0;i<n;i++){
    OLED_ShowCHinese(x+i*16,y,num+i);
  }
}
